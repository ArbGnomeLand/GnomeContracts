...
contract GnomeBondingCalculator is IBondingCalculator {
    ...
    function markdown(address _pair) external view returns (uint) {
        ...
        uint reserve;
        if (IUniswapV2Pair(_pair).token0() == address(Gnome)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(Gnome), "not a Gnome lp pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10 ** Gnome.decimals())).div(getTotalValue(_pair));
    }
}
