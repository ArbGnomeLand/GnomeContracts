...
interface IClaim {
    struct Term {
        ...
        uint256 max; // maximum nominal GNOME amount can claim
    }
...
/**
 *  This contract allows Guitsa genesis contributors to claim GNOME. It has been
...
contract GenesisClaim is Ownable {
    ...
    // claim token
    IERC20 internal immutable gnome =
    ...
    ITreasury internal immutable treasury =
        ITreasury(0x9A315BdF513367C0377FB36545857d12e85813Ef);
    // stake GNOME for sGNOME
    IStaking internal immutable staking =
        IStaking(0xB63cac384247597756545b500253ff8E607a8020);
    ...
    IgGNOME internal immutable gGNOME =
        IgGNOME(0x0ab87046fBb341D058F17CBC4c1133F25a20a52f);
    // previous deployment of contract (to migrate terms)
    IClaim internal immutable previous =
        IClaim(0xEaAA9d97Be33a764031eDdEbA1cB6Cb385350Ca3);
    ...
    /**
     * @notice allows wallet to claim GNOME
     * @param _to address
     * @param _amount uint256
     */
    function claim(address _to, uint256 _amount) external {
        gnome.safeTransfer(_to, _claim(_amount));
    }

    /**
     * @notice allows wallet to claim GNOME and stake. set _claim = true if warmup is 0.
     * @param _to address
     * @param _amount uint256
    ...
    /**
     * @notice logic for claiming GNOME
     * @param _amount uint256
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        require(
            redeemableFor(msg.sender).div(1e9) >= toSend_,
            "Claim more than vested"
        );
        require(
            info.max.sub(claimed(msg.sender)) >= toSend_,
            "Claim more than max"
        );

        if (useStatic) {
            terms[msg.sender].gClaimed = info.gClaimed.add(
                gGNOME.balanceTo(toSend_.mul(9).div(10))
            );
            terms[msg.sender].claimed = info.claimed.add(toSend_.div(10));
        } else
            terms[msg.sender].gClaimed = info.gClaimed.add(
                gGNOME.balanceTo(toSend_)
            );
    }
    ...
    function approve() external {
        gnome.approve(address(staking), 1e33);
    ...
    /**
     * @notice view GNOME claimable for address. DAI decimals (18).
    ...
    /**
     * @notice view GNOME claimed by address. GNOME decimals (9).
     * @param _address address
     * @return uint256
     */
    function claimed(address _address) public view returns (uint256) {
        return
            gGNOME.balanceFrom(terms[_address].gClaimed).add(
                terms[_address].claimed
            );
    }

    /**
     * @notice view circulating supply of GNOME
     * @notice calculated as total supply minus DAO holdings
     * @return uint256
     */
    function circulatingSupply() public view returns (uint256) {
        return treasury.baseSupply().sub(gnome.balanceOf(dao));
    }
    ...
    function setTerms(
        ...
        require(
            totalAllocated.add(_percent) <= maximumAllocated,
