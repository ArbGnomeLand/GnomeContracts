...
import {IgGNOME} from "../interfaces/IgGNOME.sol";
...
contract YieldStreamer is IYieldStreamer, YieldSplitter {
    ...
    address public immutable GNOME;
    ...
    /**
        @notice Constructor
        @param gGNOME_ Address of gGNOME.
        @param sGNOME_ Address of sGNOME.
        @param GNOME_ Address of GNOME.
        @param streamToken_ Address of the token the ohm will be swapped to.
        @param sushiRouter_ Address of sushiswap router.
        @param staking_ Address of sGNOME staking contract.
        @param authority_ Address of Gnome authority contract.
        @param maxSwapSlippagePercent_ Maximum acceptable slippage when swapping GNOME to streamTokens as percentage 6 decimals.
    ...
    */
    constructor(
    ...
    ) YieldSplitter(sGNOME_, authority_) {
        gGNOME = gGNOME_;
        GNOME = GNOME_;
        streamToken = streamToken_;
        sushiRouter = IUniswapV2Router(sushiRouter_);
        staking = IStaking(staking_);
        priceConverterOracleWrapper = AggregatorV3Interface(
            priceConverterOracleWrapper_
        );
        sushiRouterPath[0] = GNOME;
        ...
        IERC20(gGNOME).approve(address(staking), type(uint256).max);
        IERC20(GNOME).approve(address(sushiRouter), type(uint256).max);
    }

    /**
        @notice Deposit gGNOME, creates a deposit in the active deposit pool to be unkept.
        @param amount_ Amount of gGNOME.
    ...
    */
    function deposit(
        ...
        if (userMinimumAmountThreshold_ < minimumTokenThreshold)
            revert YieldStreamer_MinTokenThresholdTooLow();
    ...
    /**
        @notice Add more gGNOME to your principal deposit.
        @param id_ Id of the deposit.
        @param amount_ Amount of gGNOME to add.
    */
    ...
    /**
        ...
        @param id_ Id of the deposit.
        @param amount_ Amount of gGNOME to withdraw.
    */
    function withdrawPrincipal(uint256 id_, uint256 amount_) external override {
        ...
        if (
            amount_ <
            IgGNOME(gGNOME).balanceTo(depositInfo[id_].principalAmount)
        ) {
            _withdrawPrincipal(id_, amount_, msg.sender);
            IERC20(gGNOME).safeTransfer(msg.sender, amount_);
        } else {
            address recipient = recipientInfo[id_].recipientAddress;
            uint256 unclaimedStreamTokens = recipientInfo[id_]
                .unclaimedStreamTokens;
            (uint256 principal, uint256 totalGGNOME) = _closeDeposit(
                id_,
                msg.sender
            );
            ...
            for (uint256 i = 0; i < recipientLength; i++) {
                ...
                if (recipientIdsArray[i] == id_) {
                    recipientIdsArray[i] = recipientIdsArray[
                        recipientLength - 1
                    ];
            ...
            IERC20(gGNOME).safeTransfer(msg.sender, principal);
            IERC20(gGNOME).safeTransfer(recipient, totalGGNOME - principal);
    ...
    /**
        @notice Withdraw excess yield from your deposit in gGNOME.
    ...
    */
    function withdrawYield(uint256 id_) external override {
        if (withdrawDisabled) revert YieldStreamer_WithdrawDisabled();
        if (recipientInfo[id_].recipientAddress != msg.sender)
            revert YieldStreamer_UnauthorisedAction();

        uint256 yield = _redeemYield(id_);
        recipientInfo[id_].lastUpkeepTimestamp = uint128(block.timestamp);

        IERC20(gGNOME).safeTransfer(msg.sender, yield);
    }

    /**
        @notice Withdraw all excess yield from your all deposits you are the recipient of in gGNOME.
        @dev  Use withdrawYieldInStreamTokens() to withdraw yield in stream tokens.
    */
    function withdrawAllYield() external override {
        ...
        for (uint256 i = 0; i < receiptIds.length; i++) {
            ...
            recipientInfo[receiptIds[i]].lastUpkeepTimestamp = uint128(
                block.timestamp
            );
        }

        IERC20(gGNOME).safeTransfer(msg.sender, total);
    }
    ...
    */
        ...
        returns (uint256 amount_)
    {
        ...
        IERC20(gGNOME).safeTransfer(
            recipientInfo[id_].recipientAddress,
            amount_
        );
    }
    ...
    */
    function updateUserMinThreshold(uint256 id_, uint128 threshold_)
        external
        override
    {
        if (threshold_ < minimumTokenThreshold)
            revert YieldStreamer_MinTokenThresholdTooLow();
        if (depositInfo[id_].depositor != msg.sender)
            revert YieldStreamer_UnauthorisedAction();
    ...
    */
    function updatePaymentInterval(uint256 id_, uint128 paymentInterval_)
        external
        override
    {
        if (depositInfo[id_].depositor != msg.sender)
    ...
    function upkeep() external override {
        ...
        for (uint256 i = 0; i < depositLength; i++) {
            ...
            if (_isUpkeepEligible(currentId)) {
                totalGGNOME += redeemableBalance(currentId);
            }
        }

        uint256 feeToDao = (totalGGNOME * feeToDaoPercent) /
            sixDecimalMaxNumber;
        IERC20(gGNOME).safeTransfer(authority.governor(), feeToDao);

        uint256 totalOhmToSwap = staking.unstake(
            address(this),
            totalGGNOME - feeToDao,
            false,
            false
        );

        (, int256 ohmOraclePrice, , , ) = priceConverterOracleWrapper
            .latestRoundData();
        uint256[] memory amounts = sushiRouter.swapExactTokensForTokens(
            totalOhmToSwap,
            (uint256(ohmOraclePrice) *
                (sixDecimalMaxNumber - maxSwapSlippagePercent)) /
                sixDecimalMaxNumber,
            sushiRouterPath,
            address(this),
            block.timestamp
        ...
        for (uint256 i = 0; i < depositLength; i++) {
            ...
            if (_isUpkeepEligible(currentId)) {
                RecipientInfo storage currentrecipientInfo = recipientInfo[
                    currentId
                ];

                currentrecipientInfo.lastUpkeepTimestamp = uint128(
                    block.timestamp
                );
                currentrecipientInfo.unclaimedStreamTokens += uint128(
                    (amounts[1] * _redeemYield(currentId)) / totalGGNOME
                );

                if (
                    currentrecipientInfo.unclaimedStreamTokens >=
                    currentrecipientInfo.userMinimumAmountThreshold
                ) {
                    uint256 streamTokensToSend = currentrecipientInfo
                        .unclaimedStreamTokens;
                    currentrecipientInfo.unclaimedStreamTokens = 0;
                    IERC20(streamToken).safeTransfer(
                        currentrecipientInfo.recipientAddress,
                        streamTokensToSend
                    );
                }
            }
    ...
    */
    function totalRedeemableBalance(address recipient_)
        ...
        view
        override
        returns (uint256 totalGGNOME)
    {
        for (uint256 i = 0; i < recipientIds[recipient_].length; i++) {
            totalGGNOME += redeemableBalance(recipientIds[recipient_][i]);
        }
    }
    ...
    /**
        ...
        @return amountOfYieldToSwap : total amount of yield in gGNOME ready to be swapped in next upkeep.
     */
    function upkeepEligibility()
        external
        view
        returns (uint256 numberOfDepositsEligible, uint256 amountOfYieldToSwap)
    {
    ...
    /**
        ...
        @param id_ Id of the deposit.
     */
    function redeemableBalance(uint256 id_)
        public
        view
        override
        returns (uint256)
    {
        return
            _getOutstandingYield(
                depositInfo[id_].principalAmount,
                depositInfo[id_].agnosticAmount
            );
    }
    ...
    /**
        ...
        @param id_ Id of the deposit.
     */
    function getPrincipalInGGNOME(uint256 id_) external view returns (uint256) {
        return IgGNOME(gGNOME).balanceTo(depositInfo[id_].principalAmount);
    }
    ...
    function _isUpkeepEligible(uint256 id_) internal view returns (bool) {
        return
            block.timestamp >=
            recipientInfo[id_].lastUpkeepTimestamp +
                recipientInfo[id_].paymentInterval;
    }
    ...
    */
    function setMaxSwapSlippagePercent(uint256 slippagePercent_)
        external
        onlyGovernor
    {
        if (slippagePercent_ > sixDecimalMaxNumber)
            revert YieldStreamer_InvalidAmount();
    ...
    */
    function setFeeToDaoPercent(uint256 feePercent_) external onlyGovernor {
        if (feePercent_ > sixDecimalMaxNumber)
            revert YieldStreamer_InvalidAmount();
    ...
    */
    function setminimumTokenThreshold(uint256 minimumTokenThreshold_)
        external
        onlyGovernor
    {
        minimumTokenThreshold = minimumTokenThreshold_;
