...
import {IsGNOME} from "../interfaces/IsGNOME.sol";
...
/**
    ...
    @notice This contract allows donors to deposit their gGNOME and donate their rebases
            to any address. Donors will be able to withdraw the sGNOME equivalent of their principal
            gGNOME at any time. Donation recipients can also redeem accrued rebases at any time.
...
contract YieldDirector is IYieldDirector, YieldSplitter {
    ...
    address public immutable sGNOME;
    address public immutable gGNOME;
    ...
    constructor(
        ...
        IERC20(sGNOME).safeApprove(address(staking), type(uint256).max);
    }

    /************************
     * Modifiers
     ************************/
    function isInvalidDeposit(uint256 amount_, address recipient_)
        internal
        view
        returns (bool)
    {
    ...
    function isInvalidUpdate(uint256 depositId_, uint256 amount_)
        internal
        view
        returns (bool)
    {
        return
            depositDisabled ||
            amount_ == 0 ||
            depositInfo[depositId_].depositor == address(0);
    }
    ...
    /**
        @notice Deposit gGNOME, records sender address and assign rebases to recipient
        @param amount_ Amount of gGNOME debt issued from donor to recipient
        @param recipient_ Address to direct staking yield and vault shares to
    */
    function deposit(uint256 amount_, address recipient_)
        external
        override
        returns (uint256 depositId)
    {
        ...
        IERC20(gGNOME).safeTransferFrom(msg.sender, address(this), amount_);
    }

    /**
        @notice Deposit sGNOME, wrap to gGNOME, and records sender address and assign rebases to recipeint
        @param amount_ Amount of sGNOME debt issued from donor to recipient
        @param recipient_ Address to direct staking yield and vault shares to
    */
    function depositSohm(uint256 amount_, address recipient_)
        external
        override
        returns (uint256 depositId)
    {
        ...
        IERC20(sGNOME).safeTransferFrom(msg.sender, address(this), amount_);
        staking.wrap(address(this), amount_);
    }

    /**
        @notice Deposit additional gGNOME, and update deposit record
        @param depositId_ Deposit ID to direct additional gGNOME to
        @param amount_ Amount of new gGNOME debt issued from donor to recipient
    */
    function addToDeposit(uint256 depositId_, uint256 amount_)
        external
        override
    {
    ...
    /**
        @notice Deposit additional sGNOME, wrap to gGNOME, and update deposit record
        @param depositId_ Deposit ID to direct additional gGNOME to
        @param amount_ Amount of new sGNOME debt issued from donor to recipient
    */
    function addToSohmDeposit(uint256 depositId_, uint256 amount_)
        external
        override
    {
        uint256 gohmAmount = _toAgnostic(amount_);
    ...
    /**
        @notice Withdraw donor's gGNOME from vault
        @param depositId_ Deposit ID to remove gGNOME deposit from
        @param amount_ Amount of gGNOME deposit to remove and return to donor
    */
    function withdrawPrincipal(uint256 depositId_, uint256 amount_)
        external
        override
    {
        uint256 amountWithdrawn = _withdraw(depositId_, amount_);

        IERC20(gGNOME).safeTransfer(msg.sender, amountWithdrawn);
    }

    /**
        @notice Withdraw donor's gGNOME from vault, and return it as sGNOME
        @param depositId_ Deposit ID to remove gGNOME debt from
        @param amount_ Amount of gGNOME debt to remove and return to donor as sGNOME
    */
    function withdrawPrincipalAsSohm(uint256 depositId_, uint256 amount_)
        external
        override
    {
        uint256 amountWithdrawn = _withdraw(depositId_, amount_);
    ...
    /**
        @notice Withdraw all gGNOME from all donor positions
    */
    function withdrawAll() external override {
        ...
        IERC20(gGNOME).safeTransfer(msg.sender, agnosticAmount);
    }
    ...
    /**
        @notice Get deposited gGNOME amounts for specific recipient (updated to current index
                based on sGNOME equivalent amount deposit)
        @param donor_ Address of user donating yield
        @param recipient_ Address of user receiving donated yield
    */
    function depositsTo(address donor_, address recipient_)
        external
        view
        override
        returns (uint256)
    ...
    /**
        @notice Return total amount of donor's gGNOME deposited (updated to current index based
                on sGNOME equivalent amount deposited)
        @param donor_ Address of user donating yield
    */
    function totalDeposits(address donor_)
        external
        view
        override
        returns (uint256)
    ...
    /**
        @notice Return arrays of donor's recipients and deposit amounts (gGNOME value based on
                sGNOME equivalent deposit), matched by index
        @param donor_ Address of user donating yield
    */
    function getAllDeposits(address donor_)
        external
        view
        override
        returns (address[] memory, uint256[] memory)
    {
        ...
        if (depositIds.length == 0) {
            ...
            for (uint256 index = 0; index < len; ++index) {
                ...
                agnosticDeposits[index] = _toAgnostic(
                    depositInfo[depositIds[index]].principalAmount
                );
            }
        }
    ...
    /**
        @notice Return total amount of gGNOME donated to recipient since last full redemption
        @param donor_ Address of user donating yield
        @param recipient_ Address of user recieiving donated yield
    */
    function donatedTo(address donor_, address recipient_)
        external
        view
        override
        returns (uint256)
    ...
    /**
        @notice Return total amount of gGNOME donated from donor since last full redemption
        @param donor_ Address of user donating yield
    */
    function totalDonated(address donor_)
        external
        view
        override
        returns (uint256)
    ...
    /**
        @notice Get redeemable gGNOME balance of a specific deposit
    ...
    */
        ...
        returns (uint256)
    {
        ...
        return
            _getOutstandingYield(
                currDeposit.principalAmount,
                currDeposit.agnosticAmount
            );
    }

    /**
        @notice Get redeemable gGNOME balance of a recipient address
        @param recipient_ Address of user receiving donated yield
     */
    ...
    */
    function getRecipientIds(address recipient_)
        external
        view
        override
    ...
    /**
        @notice Redeem recipient's donated amount of sGNOME at current index from one donor as gGNOME
        @param depositId_ Deposit ID for this donation
    */
    function redeemYield(uint256 depositId_) external override {
        uint256 amountRedeemed = _redeem(depositId_, msg.sender);

        IERC20(gGNOME).safeTransfer(msg.sender, amountRedeemed);
    }

    /**
        @notice Redeem recipient's donated amount of sGNOME at current index
        @param depositId_ Deposit id for this donation
    */
    ...
    /**
        @notice Redeem recipient's full donated amount of sGNOME at current index as gGNOME
    */
    ...
    */
    ...
    {
        if (!hasPermissionToRedeem[msg.sender])
            revert YieldDirector_NotYourYield();

        address recipient = recipientLookup[id_];

        amount_ = _redeem(id_, recipient);

        IERC20(gGNOME).safeTransfer(recipient, amount_);
    }
    ...
    */
        ...
        returns (uint256 amount_)
    {
        ...
        IERC20(gGNOME).safeTransfer(recipient_, amount_);
    }
    ...
    /**
        @notice Creates a new deposit directing the yield from the deposited gGNOME amount
                to the prescribed recipient
        @param amount_ Quantity of gGNOME deposited redirecting yield to the recipient
        @param recipient_ The address of the user who will be entitled to claim the donated yield
    */
    function _createDeposit(uint256 amount_, address recipient_)
        internal
        returns (uint256 depositId)
    {
        if (isInvalidDeposit(amount_, recipient_))
            revert YieldDirector_InvalidDeposit();
    ...
    /**
        @notice Increases the amount of gGNOME directing yield to a recipient
        @param depositId_ The global ID number of the deposit to add the additional deposit to
        @param amount_ Quantity of new gGNOME deposited redirecting yield to the current deposit's recipient
    */
    function _increaseDeposit(uint256 depositId_, uint256 amount_) internal {
        if (isInvalidUpdate(depositId_, amount_))
            revert YieldDirector_InvalidUpdate();
    ...
    /**
        @notice Withdraw gGNOME deposit from vault
        @param depositId_ Deposit ID to remove gGNOME deposit from
        @param amount_ Amount of gGNOME deposit to remove and return to donor 
    */
    function _withdraw(uint256 depositId_, uint256 amount_)
        internal
        returns (uint256 amountWithdrawn)
    {
        if (isInvalidWithdrawal(amount_))
            revert YieldDirector_InvalidWithdrawal();
    ...
    /**
        @notice Redeem available gGNOME yield from a specific deposit
        @param depositId_ Deposit ID to withdraw gGNOME yield from
        @param recipient_ address of recipient
    */
    function _redeem(uint256 depositId_, address recipient_)
        internal
        returns (uint256 amountRedeemed)
    {
        if (redeemDisabled) revert YieldDirector_RedeemsDisabled();
        if (recipientLookup[depositId_] != recipient_)
            revert YieldDirector_NotYourYield();
        ...
        emit Donated(
            depositInfo[depositId_].depositor,
            recipient_,
            amountRedeemed
        );
    }
    ...
    /**
        @notice Redeem all available gGNOME yield from the vault
        @param recipient_ address of recipient
    */
        ...
        returns (uint256 amountRedeemed)
    {
        ...
        for (uint256 index = receiptIds.length; index > 0; index--) {
            ...
            address currDepositor = depositInfo[receiptIds[currIndex]]
                .depositor;
