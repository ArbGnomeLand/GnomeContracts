...
contract GnomeTreasury is GnomeAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    IGNOME public immutable GNOME;
    IsGNOME public sGNOME;
    ...
    /**
     * @notice allow approved address to deposit an asset for GNOME
    ...
    function deposit(
        ...
        // mint GNOME needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        GNOME.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn GNOME for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        GNOME.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new GNOME using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        GNOME.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or GNOME from the treasury, using sGNOME as collateral. This might allow an
     * sGNOME holder to provide GNOME liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(GNOME)) {
            require(permissions[STATUS.GNOMEDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sGNOME.changeDebt(value, msg.sender, true);
        require(
            sGNOME.debtBalances(msg.sender) <= debtLimit[msg.sender],
            "Treasury: exceeds limit"
        );
        totalDebt = totalDebt.add(value);

        if (_token == address(GNOME)) {
            GNOME.mint(msg.sender, value);
            gnomeDebt = gnomeDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
        ...
        override
    {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sGNOME.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with GNOME
     * @param _amount uint256
     */
    function repayDebtWithGNOME(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] ||
                permissions[STATUS.GNOMEDEBTOR][msg.sender],
            notApproved
        );
        GNOME.burnFrom(msg.sender, _amount);
        sGNOME.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        gnomeDebt = gnomeDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(GNOME), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SGNOME) {
            sGNOME = IsGNOME(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SGNOME) {
            // 9
            sGNOME = IsGNOME(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(GNOME.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns GNOME valuation of asset
     * @param _token address
     * @param _amount uint256
    ...
    {
        value_ = _amount.mul(10**IERC20Metadata(address(GNOME)).decimals()).div(
    ...
    function baseSupply() external view override returns (uint256) {
        return GNOME.totalSupply() - gnomeDebt;
    }
}
