...
contract sGnome is IsGNOME, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgGNOME public gGNOME; // additional staked supply (governance token)
    ...
    constructor()
        ERC20("Staked GNOME", "sGNOME", 9)
        ERC20Permit("Staked GNOME")
    {
    ...
    function setgGNOME(address _gGNOME) external {
        require(
            msg.sender == initializer,
            "Initializer:  caller is not initializer"
        );
        require(address(gGNOME) == address(0), "gGNOME:  gGNOME already set");
        require(
            _gGNOME != address(0),
            "gGNOME:  gGNOME is not a valid contract"
        );
    ...
    /**
        @notice increases rGNOME supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sGNOME of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sGNOME
    ...
    // toG converts an sGNOME balance to gGNOME terms. gGNOME is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gGNOME.balanceTo(amount);
    }

    // fromG converts a gGNOME balance to sGNOME terms. sGNOME is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gGNOME.balanceFrom(amount);
    }

    // Staking contract holds excess sGNOME
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gGNOME.balanceFrom(IERC20(address(gGNOME)).totalSupply()))
