...
abstract contract GnomeAccessControlledV2 {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(IGnomeAuthority authority);
    ...
    function initializeAuthority(IGnomeAuthority _newAuthority) internal {
        if (authority != IGnomeAuthority(address(0)))
            revert AUTHORITY_INITIALIZED();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }

    function setAuthority(IGnomeAuthority _newAuthority) external {
        _onlyGovernor();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }
