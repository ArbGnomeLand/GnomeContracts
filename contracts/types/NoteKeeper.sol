...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
     ...
     * @param _payout      the amount of GNOME due to the user
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({
                payout: gGNOME.balanceTo(_payout),
        ...
        // note that only the payout gets staked (front end rewards are in GNOME)
     ...
     * @param _sendgGNOME    send payout as gGNOME or sGNOME
     * @return payout_     sum of payout sent, in gGNOME
     */
    function redeem(
        address _user,
        ...
        if (_sendgGNOME) {
            gGNOME.transfer(_user, payout_); // send payout as gGNOME
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as sGNOME
        }
    }
     ...
     * @param _sendgGNOME    send payout as gGNOME or sGNOME
     * @return             sum of payout sent, in gGNOME
     */
    function redeemAll(address _user, bool _sendgGNOME)
        external
        override
        returns (uint256)
    {
        return redeem(_user, indexesFor(_user), _sendgGNOME);
    }
     ...
     * @return payout_     the payout due, in gGNOME
