...
import {GnomeAccessControlledV2, IGnomeAuthority} from "../types/GnomeAccessControlledV2.sol";

/**
    @title IGNOMEIndexWrapper
    @notice This interface is used to wrap cross-chain oracles to feed an index without needing IsGNOME, 
    while also being able to use sGNOME on mainnet.
 */
interface IGNOMEIndexWrapper {
...
/**
    @title YieldSplitter
    @notice Abstract contract that allows users to create deposits for their gGNOME and have
            ...
            emergency controls, sending and recieving gGNOME is up to the implementation of
            this abstract contract to handle.
 */
abstract contract YieldSplitter is GnomeAccessControlledV2, IYieldSplitter {
    using SafeERC20 for IERC20;

    IGNOMEIndexWrapper public immutable indexWrapper;
    ...
    /**
        @notice Constructor
        @param indexWrapper_ Address of contract that will return the sGNOME to gGNOME index. 
                             On mainnet this will be sGNOME but on other chains can be an oracle wrapper.
    */
    constructor(address indexWrapper_, address authority_)
        GnomeAccessControlledV2(IGnomeAuthority(authority_))
    {
        indexWrapper = IGNOMEIndexWrapper(indexWrapper_);
    }
    ...
    */
    ...
    /**
        ...
        @param id_ Id of the deposit.
        @return amountWithdrawn : amount of gGNOME withdrawn. 18 decimals.
    */
    ...
    /**
        @notice Redeem excess yield from your deposit in sGNOME.
        @param id_ Id of the deposit.
        @return amountRedeemed : amount of yield redeemed in gGNOME. 18 decimals.
    */
    ...
    /**
        ...
        @param id_ Id of the deposit.
        @dev Internally for accounting reasons principal amount is stored in 9 decimal GNOME terms. 
        Since most implementations will work will gGNOME, principal here is returned externally in 18 decimal gGNOME terms.
        @return principal : amount of principal that was deleted. in gGNOME. 18 decimals.
        @return agnosticAmount : total amount of gGNOME deleted. Principal + Yield. 18 decimals.
    */
    ...
    */
    ...
    /**
        ...
        @return uint256 amount of yield in gGNOME. 18 decimals.
     */
    ...
    /**
        @notice Convert flat sGNOME value to agnostic gGNOME value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e18 is because sGNOME has 9 decimals, gGNOME has 18 and index has 9.
     */
    ...
    /**
        @notice Convert agnostic gGNOME value at current index to flat sGNOME value
        @dev Agnostic value earns rebases. sGNOME amount is gGNOMEamount * rebase_index.
             1e18 is because sGNOME has 9 decimals, gGNOME has 18 and index has 9.
     */
