...
contract GnomeBondDepository is GnomeAccessControlled {
    ...
    function maxPayout(uint256 _BID) public view returns (uint256) {
        return
            treasury.baseSupply().mul(bonds[_BID].terms.maxPayout).div(100000);
    }
     ...
     * @return uint
     */
    function payoutFor(uint256 _value, uint256 _BID)
        public
        view
        returns (uint256)
    {
        return
            FixedPoint.fraction(_value, bondPrice(_BID)).decode112with18().div(
                1e16
            );
    }
     ...
     * @param _BID uint
     */
    function payoutForAmount(uint256 _amount, uint256 _BID)
        public
        view
        returns (uint256)
    {
    ...
    function bondPriceInUSD(uint256 _BID) public view returns (uint256 price_) {
        ...
        if (address(bond.calculator) != address(0)) {
            price_ = bondPrice(_BID)
                .mul(bond.calculator.markdown(address(bond.principal)))
                .div(100);
        } else {
            price_ = bondPrice(_BID)
                .mul(10**IERC20Metadata(address(bond.principal)).decimals())
                .div(100);
        }
    }
    ...
    function debtRatio(uint256 _BID) public view returns (uint256 debtRatio_) {
        debtRatio_ = FixedPoint
            .fraction(currentDebt(_BID).mul(1e9), treasury.baseSupply())
            .decode112with18()
            .div(1e18);
    }
    ...
    function debtDecay(uint256 _BID) public view returns (uint256 decay_) {
        ...
        decay_ = bond.totalDebt.mul(blocksSinceLast).div(
