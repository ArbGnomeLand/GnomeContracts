...
interface IGNOMEERC20 is IERC20Mintable, IERC20 {
...
contract GnomeTreasury is Ownable {
    ...
    IGNOMEERC20 public immutable Gnome;
    ...
    constructor(address _Gnome, address _MIM, uint32 _secondsNeededForQueue, uint256 _limitAmount) {
        require(_Gnome != address(0));
        Gnome = IGNOMEERC20(_Gnome);
    ...
    function incurDebt(uint _amount, address _token) external {
        ...
        uint maximumDebt = MEMEs.balanceOf(msg.sender); // Can only borrow against sOHM held
    ...
    function valueOf(address _token, uint _amount) public view returns (uint value_) {
        if (isReserveToken[_token]) {
            // convert amount to match Gnome decimals
            value_ = _amount.mul(10 ** Gnome.decimals()).div(10 ** IERC20(_token).decimals());
