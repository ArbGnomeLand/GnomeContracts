...
library Address {
    ...
    function addressToString(address _address) internal pure returns (string memory) {
        uint256 addy = uint256(uint160(_address));
        bytes32 _bytes = bytes32(uint256(addy));
...
contract GnomeBondDepository is Ownable {
    ...
    IERC20 public immutable Gnome; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Gnome when receives principle
    ...
    struct Bond {
        uint payout; // Gnome remaining to be paid
        uint pricePaid; // In DAI, for front end viewing
        uint32 lastGnome; // Last interaction
    ...
    struct Adjust {
        ...
        uint32 lastGnome; // time when last adjustment made
    }

    /* ======== INITIALIZATION ======== */

    constructor(address _Gnome, address _principle, address _treasury, address _DAO, address _bondCalculator) {
        require(_Gnome != address(0));
        Gnome = IERC20(_Gnome);
    ...
    function setBondTerms(PARAMETER _parameter, uint _input) external onlyOwner {
        if (_parameter == PARAMETER.VESTING) {
            // 0
            require(_input >= 1296000, "Vesting must be longer than 36 hours");
    ...
    function setAdjustment(bool _addition, uint _increment, uint _target, uint32 _buffer) external onlyOwner {
        ...
        adjustment = Adjust({
            ...
            target: _target,
            buffer: _buffer,
            lastGnome: uint32(block.timestamp)
        });
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 Gnome ( underflow protection )
        ...
        uint balanceBefore = Gnome.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Gnome
         */
        ...
        if (fee != 0) {
            ...
            Gnome.safeTransfer(DAO, fee);
        }
        require(balanceBefore.add(profit) == Gnome.balanceOf(address(this)), "Not enough Gnome to cover profit");
    ...
    function redeem(address _recipient, bool _stake) external returns (uint) {
        ...
        if (percentVested >= 10000) {
            ...
            bondInfo[_recipient] = Bond({
                ...
                vesting: info.vesting.sub32(uint32(block.timestamp).sub32(info.lastGnome)),
                lastGnome: uint32(block.timestamp),
    ...
    function stakeOrSend(address _recipient, bool _stake, uint _amount) internal returns (uint) {
        if (!_stake) {
            ...
            Gnome.transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                Gnome.approve(address(stakingHelper), _amount);
                stakingHelper.stake(_amount, _recipient);
            } else {
                Gnome.approve(address(staking), _amount);
    ...
    function adjust() internal {
        uint timeCanAdjust = adjustment.lastGnome.add32(adjustment.buffer);
        if (adjustment.rate != 0 && block.timestamp >= timeCanAdjust) {
            ...
            adjustment.lastGnome = uint32(block.timestamp);
    ...
    function maxPayout() public view returns (uint) {
        return Gnome.totalSupply().mul(terms.maxPayout) / 100000;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Gnome supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint debtRatio_) {
        uint supply = Gnome.totalSupply();
    ...
    function percentVestedFor(address _depositor) public view returns (uint percentVested_) {
        ...
        uint secondsSinceLast = uint32(block.timestamp).sub32(bond.lastGnome);
    ...
    /**
     *  @notice calculate amount of Gnome available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Gnome) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token) external returns (bool) {
        require(_token != Gnome, "NAT");
