...
contract GnomeStaking is Ownable {
    using LowGasSafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IMeme;

    IERC20 public immutable Gnome;
    IMeme public immutable Memes;
    ...
    constructor(address _Gnome, address _Memes, uint32 _epochLength, uint _firstEpochNumber, uint32 _firstEpochTime) {
        require(_Gnome != address(0));
        Gnome = IERC20(_Gnome);
        require(_Memes != address(0));
        Memes = IMeme(_Memes);
    ...
    function stake(uint _amount, address _recipient) external returns (bool) {
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons.add(Memes.gonsForBalance(_amount)),
            expiry: epoch.number.add(warmupPeriod),
            lock: false
        });

        Memes.safeTransfer(address(warmupContract), _amount);
    ...
    function claim(address _recipient) external {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            uint256 amount = Memes.balanceForGons(info.gons);
    ...
    /**
        @notice forfeit MEMO in warmup and retrieve Gnome
     */
    function forfeit() external {
        Claim memory info = warmupInfo[msg.sender];
        delete warmupInfo[msg.sender];
        uint memeBalance = Memes.balanceForGons(info.gons);
        warmupContract.retrieve(address(this), memeBalance);
        Gnome.safeTransfer(msg.sender, info.deposit);
        emit LogForfeit(msg.sender, memeBalance, info.deposit);
    }
    ...
    function unstake(uint _amount, bool _trigger) external {
        ...
        Memes.safeTransferFrom(msg.sender, address(this), _amount);
        Gnome.safeTransfer(msg.sender, _amount);
    ...
    function rebase() public {
        if (epoch.endTime <= uint32(block.timestamp)) {
            Memes.rebase(epoch.distribute, epoch.number);
            ...
            uint staked = Memes.circulatingSupply();
    ...
    /**
        @notice returns contract Gnome holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns (uint) {
        return Gnome.balanceOf(address(this)).add(totalBonus);
    }
